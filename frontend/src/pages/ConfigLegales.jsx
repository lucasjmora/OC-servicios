...
import { getConfigLegales, updateConfigLegales } from '../services/api';
...
const ConfigLegales = () => {
  const [config, setConfig] = useState({
    ...
    rutaAdjuntos: 'C:\\legales\\adjuntos'
  });
  ...
  const loadConfig = async () => {
    try {
      setLoading(true);
      const response = await getConfigLegales();
      
      if (response.data) {
        setConfig({
          diasSinComentarios: response.data.diasSinComentarios || 7,
          rutaAdjuntos: response.data.rutaAdjuntos || 'C:\\legales\\adjuntos'
        });
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      await updateConfigLegales({
        diasSinComentarios: parseInt(config.diasSinComentarios),
        rutaAdjuntos: config.rutaAdjuntos.trim()
      });
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageHeader
        title="Parámetros de Legales"
        subtitle="Configuración de parámetros para el sistema de casos legales"
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-background-card border border-gray-700 rounded-lg p-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Ruta de almacenamiento de archivos adjuntos
              </label>
              <input
                type="text"
                value={config.rutaAdjuntos}
                onChange={(e) => handleInputChange('rutaAdjuntos', e.target.value)}
                ...
                placeholder="C:\\legales\\adjuntos"
              />
              <p className="text-sm text-gray-400 mt-2">
                Ruta completa donde se almacenarán los archivos adjuntos de los casos legales.
              </p>
            </div>
