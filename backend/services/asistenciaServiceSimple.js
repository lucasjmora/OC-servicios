...
/**
 * Servicio simple: datos reales con muestra muy pequeña (10 citas)
 */
export async function getCitasConAsistencia(filters = {}, pagination = {}) {
  try {
    ...
    const {
      page = 1,
      limit = 50 // Aumentar límite para mostrar más citas
    } = pagination;
    ...
    console.log(`📊 Servicio SIMPLE - Estado: ${estadoAsistencia}`);
    ...
    // Procesar cada cita de forma simple
    ...
    for (const cita of citas) {
      ...
      // Verificar asistencia solo si tiene fecha y matrícula
      if (cita['Fecha ci'] && cita.Matricula) {
        try {
          ...
          if (!isNaN(fechaCita.getTime())) {
            ...
            // Buscar ingreso con timeout muy corto
            const ingresoEncontrado = await Ingreso.findOne({
              'Matrícula vehí': cita.Matricula,
              Fecaper: {
                $gte: fechaCita,
                $lte: fechaLimite
              }
            }).select('Referencia Fecaper').lean().maxTimeMS(2000);
    ...
    console.log(`📊 Resumen SIMPLE: ${conAsistencia} asistieron, ${sinAsistencia} no asistieron`);
    console.log(`📊 Filtro "${estadoAsistencia}" -> ${citasFiltradas.length} citas mostradas`);

    // Calcular paginación correcta
    ...
    console.error('Error en servicio SIMPLE:', error);
    throw error;
  }
}
