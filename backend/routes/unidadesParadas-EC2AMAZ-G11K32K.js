...
router.post('/:referencia/comentarios', async (req, res) => {
  try {
    ...
    if (unidadParada.estado === 'abierto' && unidadParada.subEstado === 'pendiente') {
...
router.put('/:referencia/alarma', async (req, res) => {
  try {
    ...
    if (unidadParada.estado !== 'abierto') {
      return res.status(400).json({ error: 'Solo se pueden configurar alarmas para unidades paradas con estado "abierto"' });
    }
