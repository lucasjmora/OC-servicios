...
router.get('/', async (req, res) => {
  try {
    ...
    // Ordenar por prioridad: "a_tratar" primero, luego por fecha
    oportunidadesCompletas.sort((a, b) => {
      if (a.estado === 'a_tratar' && b.estado !== 'a_tratar') return -1;
      if (b.estado === 'a_tratar' && a.estado !== 'a_tratar') return 1;
      return new Date(b['F cierr']) - new Date(a['F cierr']);
    });
    
    // Aplicar paginación
    const total = oportunidadesCompletas.length;
    const skip = (Number(page) - 1) * Number(limit);
    const ingresosPaginados = oportunidadesCompletas.slice(skip, skip + Number(limit));
...
router.put('/:referencia/estado', async (req, res) => {
  try {
    ...
    if (estado === 'cerrado' && (!motivo || motivo.trim() === '')) {
      return res.status(400).json({ error: 'El motivo de cierre es obligatorio' });
    }
    ...
    await registrarLog(
      ...
      estado,
      usuario,
      estado === 'cerrado' ? `Cerrado: ${motivo}` : `Estado cambiado a ${estado}`
    );
...
router.put('/:referencia/alarma', async (req, res) => {
  try {
    ...
    const alarmaAnterior = oportunidad.alarma;
    oportunidad.alarma = {
    ...
    await registrarLog(
      ...
      oportunidad.estado,
      usuario,
      `Alarma configurada para ${fechaAlarma.toLocaleString('es-ES')}`
    );
...
router.post('/:referencia/comentarios', async (req, res) => {
  try {
    ...
    // Transición automática a "en_gestion" si está en "pendiente"
